      ...
      <Card className="shadow p-4 mt-4">
        ...
        <Form onSubmit={handleSubmit}>
            ...
            <Row>
              {['scheduled', 'rescheduled', 'cancelled', 'noshow'].map((status) => (
                <Col xs={6} md={3} key={status}>
                  <Form.Check  type="checkbox"  label={status.charAt(0).toUpperCase() + status.slice(1)} name="interviewDate" value={status} checked={formInput.interviewDate.includes(status)} onChange={changeInput}   />  </Col>
              ))}
            </Row>
          </Form.Group>
          ...
          <div className="d-flex justify-content-between">
            ...
            <Link to="/view" className="btn btn-secondary">   Back to View  </Link>
          </div>
        </Form>
      </Card>
    </Container>
